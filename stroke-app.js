...
// Called from each stroke page after initApp
function initVideos(strokeKey) {
  // Inject after a short delay so DOM is ready
  setTimeout(function() { 
    if (typeof injectDrillVideos === 'function') injectDrillVideos(strokeKey);
  }, 200);
  // Re-inject when user navigates to drills page
  var orig = window.goPage;
  window.goPage = function(pid, nbid) {
    orig(pid, nbid);
    if (pid === 'p-drills') {
      setTimeout(function() {
        if (typeof injectDrillVideos === 'function') injectDrillVideos(strokeKey);
